...
public class MediaFile {
    ...
    public static boolean isPVMedia(String path) {
        int fileType = 0;
        try {
            fileType = getFileType(path).fileType;
		} catch (Exception e) {
			// NOTHING
		}
    ...
    public static boolean isVideoMedia(String path) {
        MediaFileType mt = getFileType(path);
        if (mt == null) {
            return false;
        } else {
            int fileType = mt.fileType;
            return isVideoFileType(fileType);
        }
    }
